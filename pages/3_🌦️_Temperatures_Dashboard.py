...
min_temp = min(temps_df["AvgTemperatureCelsius"])
max_temp = max(temps_df["AvgTemperatureCelsius"])
...
if unique_countries_list is not None and len(selected_cities) > 0:
    ...
    plt.title("Average Temperature for {selected_cities}")   # TODO
    plt.xlabel("Date")  # TODO
    plt.ylabel("Average Temperature")  # TODO
    ...
    for city in selected_cities:
        city_df = temps_df[temps_df["City"]==city]             # TODO
        city_df_period = city_df[(city_df["Date"]>=start_date) & (city_df["Date"]<=end_date)]     # TODO
        plt.hist(city_df_period["AvgTemperatureCelsius"], label=city, alpha=0.6, bins=30)                # TODO

    plt.title("Temperatures distribution")   # TODO
    plt.xlabel("Temperatures in Celsius")  # TODO
